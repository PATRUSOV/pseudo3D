...
class Camera:
    ...
    def set_normal_line(self, dist:int):
        """
        Sets the distance from the camera to the normal line.
        """
        if dist < 0:
            raise IncorrectValue("The engine does not work with a negative distance from the camera to the line of normal.")
        self._dist_to_the_normal_line = dist
    ...
    def move(self) -> None:
        self._X += np.sin(deg2rad(self._gaze_direction)) * self._speed
        self._Y += np.cos(deg2rad(self._gaze_direction)) * self._speed
